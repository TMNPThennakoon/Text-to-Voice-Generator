...
export const VoiceControls = ({ settings, availableVoices, onSettingsChange }: VoiceControlsProps) => {
  ...
  // Get Sinhala voices for presets
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          {sinhalaVoices.length > 0 && (
            <motion.div
              ...
              className="glass border border-dark-border/50 rounded-xl p-3 sm:p-4 bg-gradient-to-br from-blue-500/10 to-cyan-500/10"
            >
              <div className="flex items-center gap-2 mb-3">
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-3">
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={() => {
                    // Select first Sinhala voice (prefer male-sounding names)
                    const maleVoice = sinhalaVoices.find(v => 
                  ...
                  className="p-3 rounded-lg border border-blue-500/50 bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 transition-all flex flex-col items-center justify-center"
                  title="Sinhala Male Voice (Lower Pitch)"
                >
                  <div className="text-2xl mb-1">👨</div>
                  <div className="text-xs font-semibold">Sinhala Male</div>
                </motion.button>
                <motion.button
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => {
                    // Select Sinhala voice (prefer female-sounding names)
                    const femaleVoice = sinhalaVoices.find(v => 
                      v.name.toLowerCase().includes('female') || 
                      v.name.toLowerCase().includes('woman')
                    ) || sinhalaVoices[sinhalaVoices.length > 1 ? 1 : 0];
                    onSettingsChange({ 
                      voice: femaleVoice,
                      pitch: 1.15, // Higher pitch for female
                      rate: 1.0
                    });
                  }}
                  className="p-3 rounded-lg border border-pink-500/50 bg-pink-500/20 hover:bg-pink-500/30 text-pink-400 transition-all flex flex-col items-center justify-center"
                  title="Sinhala Female Voice (Higher Pitch)"
                >
                  <div className="text-2xl mb-1">👩</div>
                  <div className="text-xs font-semibold">Sinhala Female</div>
                </motion.button>
              </div>
              <p className="text-xs text-dark-textSecondary mt-2 text-center">
                {sinhalaVoices.length} Sinhala voice{sinhalaVoices.length !== 1 ? 's' : ''} available
              </p>
            </motion.div>
          )}
